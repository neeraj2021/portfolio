      ...
      <div className="mx-auto max-w-320 px-6">
        <div className="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-6">
          {metrics.map((m, idx) => (
            <div
              key={idx}
              className="rounded-r-lg border-l-2 border-primary/40 bg-surface-container/30 p-4 transition-all hover:border-primary"
            >
              <div className="text-2xl font-bold tracking-tight text-primary lg:text-3xl">
                {m.value}
              </div>
              <div className="mt-1 font-mono text-[11px] font-medium tracking-wider text-outline uppercase">
                {m.label}
              </div>
            </div>
          ))}
